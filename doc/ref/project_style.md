...
Предпочитать естественный порядок объявления и группировки полей в соответствии с логикой класса и
предметной области. Но если между полями нет подобных связей или нужно расположить блоки полей
относительно друг друга, можно придерживаться следующей схемы расположения полей класса:
```java
public
private
protected
...
В качестве отступа использовать половину **пробелов** как описано
в [OJCC](project_style.md#java-code-style). То есть два пробела для отступа и четыре для двойного
отступа.
...
таблицу, а лишнии переходы в строках этому мешают.
...
Во все примеры [OJCC](project_style.md#java-code-style) закралась ошибка первый `statements`
находится на следующей строчке от открывающей скобки и нет отступа в начале нового кодового блока:
```java
// Не так:
{statements;
...
...
Придерживайтесь правила если вы отступаете blank lines после объявления класса отступайте так же и
при его завершении. Или не делайте этого. Пример:
```java
// С blank lines
...
пробелов [RMCC](project_style.md#java-code-style).
...
отделяются символом `.`. Если имя пакета состоит из двух слов и это не 2 пакета придется написать их
слитно без разделителей, но лучше такого избегать.
...
Предпочитай длинные имена коротким. Длинна имени должна быть соразмерна важности и области видимости
поля [RMCC](project_style.md#java-code-style).
...
регистре. Aббревиатуры: ID, PK, FK использовать как id, pk, fk.
